# jobbot — documentation

Full documentation for the internship discovery bot. The repository
[README](README.md) is generated and shows the current openings.
...
├── alembic.ini
├── .env.example
├── README.md                       # landing page; listings table is generated
├── DOCS.md                         # this file
├── DEPLOY.md
...
named `README` (it looks in `.github/`, the repo root, then `docs/`), so the
table has to go there. Point `GITHUB_PUBLISH_PATH` at `README.md` and add these
markers wherever you want it:
...
hand-written. Without markers the target file is replaced wholesale, which suits
a dedicated listings file.

This repository uses that layout: `README.md` is a short landing page whose
marker block holds the live table, and this file (`DOCS.md`) holds the
documentation that would otherwise be buried underneath it.
